...
<div class="tour_info" id="tour_info_main">
	...
	<div class="tour_detail">
		<div class="tour_inquiry_s">
			...
			<a href="<?php get_permalink( get_page_by_path( 'inquiry' ) ); ?>">お問い合わせ</a>
		</div>
	</div><!-- .end .tour_detail -->
</div><!-- .end .tour_info -->

<div class="tour_info" id="tour_info_sub">
	<div class="sub_image">
		<?php
		$url = wp_get_attachment_image_src( $post->cf_img_1, 'sub_image' );
		if( $url ){ echo "<img class='first' src='{$url[0]}' alt=''>";}
		$url = wp_get_attachment_image_src( $post->cf_img_2, 'sub_image' );
		if( $url ){ echo "<img class='second' src='{$url[0]}' alt=''>";}
		?>
	</div>
	<h2>プランのポイント</h2>
	<div id="tour_conent_sub">
		<?php the_content(); ?>
	</div>
</div><!-- .end .tour_info -->
